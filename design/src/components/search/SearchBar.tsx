...
import { mockSearchResults, getAIResult } from '../../data/mockData'
...
export default function AISearchBar() {
  ...
  const { searchQuery, setSearchQuery, setAIResult } = useRepoStore()
  ...
  // Submit → go to AI result page
  function submitSearch(query: string) {
    if (!query.trim()) return
    const result = getAIResult(query)
  ...
  const files = filtered.filter(r => r.type === 'file')
  const fns   = filtered.filter(r => r.type === 'function')
  const syms  = filtered.filter(r => r.type === 'symbol')
      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] left-0 right-0 rounded-2xl overflow-hidden animate-fade-in z-50"
          ...
          {files.length > 0 && <>
            ...
            {files.map(r => (
              <ResultRow key={r.path} result={r} active={filtered.indexOf(r) === activeIdx}
                onHover={() => setActiveIdx(filtered.indexOf(r))} onClick={() => handleSelect(r)} query={searchQuery} />
            ))}
          </>}
          ...
          {fns.length > 0 && <>
            ...
            {fns.map(r => (
              <ResultRow key={r.path+r.name} result={r} active={filtered.indexOf(r) === activeIdx}
                onHover={() => setActiveIdx(filtered.indexOf(r))} onClick={() => handleSelect(r)} query={searchQuery} />
            ))}
          </>}
